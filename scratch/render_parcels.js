  ...
  parcels.forEach((p, idx) => {
    ...
    for (let r = 0; r <= 8; r++) {
      for (let a = 0; a < 360; a += 10) {
        ...
        setPixel(Math.round(cx + r * Math.cos(rad)), Math.round(cy + r * Math.sin(rad)), color[0], color[1], color[2], 255);
      }
    }
  });

  const outBuffer = PNG.sync.write(png);
  const outPath = 'C:\\Users\\princ\\.gemini\\antigravity-ide\\brain\\7f3d80fa-c0b6-44b1-b1b1-0f35461f2e75\\spatial_reasoning_parcels_render.png';
  fs.writeFileSync(outPath, outBuffer);
  console.log(`Rendered overlay saved successfully to ${outPath}`);
}
